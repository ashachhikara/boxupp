...
ngBoxuppApp.factory('User',function($http,$location,$q){

=======
ngBoxuppApp.factory('User',function($http,$location){
...
<<<<<<< HEAD
			...
			return $http({
		 				method: 'GET',
				 		url: '/boxupp/resources/user/signout/',
		 			}).then(function(result){
						$location.path('/');
		 				return result.data;
		 			});	
		...
		checkSession : function(){
			...
			return $http({
		 				method: 'GET',
				 		url: '/boxupp/resources/user/checkSession/',
		 			}).then(function(result){
...
>>>>>>> 6f531bc934e3b2d6681b64c4509d519436275e91
		checkIfExists : function(userMailID){
			return $http({
		 				method: 'GET',
				 		url: '/boxupp/resources/user/idCheck/' + userMailID,
...
ngBoxuppApp.factory("HostName",function($resource){
	return $resource('/boxupp/resources/machineConfig/hostName/:id',{id:'@id'},{
		update:{
			method:'PUT'
		}
	});
...
ngBoxuppApp.factory("AuthenticateAwsCred",function($resource){
	return $resource('/boxupp/resources/project/authenticateCred');
});
...
ngBoxuppApp.factory('vagrantStatus',function($http,$q,$timeout){
		return{
			...
			checkMachineStatus : function(userID,vagrantID){
				var parameters = {"userID" : userID, "vagrantID" : vagrantID};
				var completeURL = "boxupp/checkMachineStatus";
				var deferred = $q.defer();
			...
			checkAllMachineStatus : function($scope, userID, vagrantID, promise){
				if (!window.WebSocket){
				/*var parameters = {"userID" : userID, "boxesData" : boxesData};
				var completeURL = "boxupp/checkAllMachineStatus";
				var deferred = $q.defer();
				...
				error(function(data, status, headers, config) {
					...
					$scope.statusServer.connect(promise);
					$scope.waitForBoxStatusWSConnection(function(){
						$scope.statusServer.send(userID);
					});
...
ngBoxuppApp.factory('executeCommand',function($http,$q,$timeout,$routeParams){
		return{
			
			triggerVagrantCommand : function($scope,commandForMachine,promise){
				if (!window.WebSocket){
					console.log("WebSocket not supported by this browser");
					var completeURL = "/services/boxupp?command=" + commandForMachine;
					$http({		
						method:'GET',
						headers:{'Content-Type':'application/json; charset=UTF-8'},
						url:completeURL
					...
					console.log("Websocket supported");
					$scope.server.connect(promise);
					$scope.waitForWSConnection(function(){
						$scope.server.send(commandForMachine+":"+$routeParams.userID);
					});
			...
			createVagrantFile : function(projectID, userID){
				var mappings = {"projectID":projectID, "userID":userID};
				var completeURL = 'boxupp/resources/project/createVagrantFile';
				var deferred = $q.defer();
				$http({	
						method:'POST',
						headers:{'Content-Type':'application/json; charset=UTF-8'},
						url:completeURL,
						data:angular.toJson(mappings)
					}).
				success(function(data, status, headers, config) {
					if(data.fileCreated){
				...
				error(function(data, status, headers, config) {
					console.log("Error creating Vagrant file: " + data);
				});
				return deferred.promise;
			}
		}
...
ngBoxuppApp.factory('puppetModule',function($http,$q,$timeout,$routeParams,miscUtil,$resource){
	return{
...
ngBoxuppApp.factory('puppetModuleResource',function($resource){
	return $resource('/boxupp/resources/puppetModule/:id');
});
	...
	return{
		...
		selectActiveProject : function(){
			...
			$http({	
				...
				console.error("Error saving Boxupp Data: " + response);
			});
			return deferred.promise;
		}
	}
	...
	return{
		commitShellMappings : function(shellProvMappings){
			...
			$http({	
				...
				console.log("Error committing shell prov mappings: " + status);
			});
			return deferred.promise;
		},
...
ngBoxuppApp.factory('loggerFunctionality', function($http, $q, $routeParams){
	return {
			getLogFiles: function(userID, fromDate, toDate){
				var parameters = {"userID":userID,"fromDate":fromDate,"toDate":toDate};
				var completeURL = "/boxupp/resources/vagrantLog/getVagrantLogFiles";
				var deferred = $q.defer();
				$http({	
					method:'GET',
					headers:{'Content-Type':'application/json; charset=UTF-8'},
			...
			getLogFileContent: function(userID, fileName){
				var parameters = {"userID":userID,"fileName":fileName};
				var completeURL = "/boxupp/resources/vagrantLog/getVagrantLogFileContent";
				var deferred = $q.defer();
				console.log("before  casll");
				$http({	
					method:'GET',
					...
					console.log("Error saving Boxupp Data: " + status);
				});
				return deferred.promise;
			}
	}
});
ngBoxuppApp.factory('boxFunctionality', function($http,$q,$routeParams){
	return {
		stopBox : function(machineData){
			var stopBoxURL = "/boxupp/resources/machineConfig/stopMachine";
			...
			$http({	
				...
				headers : {'Content-Type':'application/json; charset=UTF-8'},
				url : stopBoxURL,
				data : angular.toJson(machineData)
			}).	success(function(response, status, headers, config) {
				console.log("Box stoped Successfully");
				deferred.resolve(response);
			}).	error(function(response, status, headers, config) {
				console.log("Error Stoping Box: " + response);
			});
			return deferred.promise;
		},
		reloadBox: function(machineData){
			var reloadBoxURL = "/boxupp/resources/machineConfig/reloadMachine";
		...
		updateMachineMapping: function(machineData){
			console.log(machineData);
			var reloadBoxURL = "/boxupp/resources/machineConfig/updateMachineMappingWithPuppet";
			var deferred = $q.defer();
			$http({	
				method : 'POST',
				headers : {'Content-Type':'application/json; charset=UTF-8'},
				...
				console.log("Box update Successfully");
				deferred.resolve(response);
			}).	error(function(response, status, headers, config) {
				console.log("Error updating  Box: " + response);
