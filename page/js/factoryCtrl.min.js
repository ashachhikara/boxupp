...
ngBoxuppApp.factory('boxes',function($http,$q){
	return {
		fetchList : function(projectID){
			return $http({
		 				method: 'GET',
				 		url: '/boxupp/resources/project/boxes/' + projectID + '/',
		 			}).then(function(result){
		 				return result.data;
		 			});		
		}
...
ngBoxuppApp.factory('puppetModule',function($http,$q,$timeout){
	return{
		searchPuppetModule : function($scope, searchModule){
			...
			var completeURL = $scope.serverAddress + "/services/searchPuppetModule";
			var deferred = $q.defer();
			$http({	
				method:'GET',
				headers:{'Content-Type':'application/json; charset=UTF-8'},
