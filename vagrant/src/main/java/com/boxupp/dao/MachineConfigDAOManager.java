...
import com.paxcel.boxupp.db.beans.manager.DockerLinkDBManager;
import com.paxcel.boxupp.db.beans.manager.PortMappingDBMAnager;
import com.paxcel.boxupp.db.beans.manager.ProjectDBManager;
import com.paxcel.boxupp.db.beans.manager.SyncFolderDBManager;
...
public class MachineConfigDAOManager implements DAOImplInterface {
	...
	public <T>T read(String id) {
		MachineConfigurationBean machineConfiguration = null;
		try{
			machineConfiguration = machineConfigDao.queryForId(Integer.parseInt(id));
		}catch(SQLException e){
			logger.error("Error querying the machine config from DB : " + e.getMessage());
		}
		return (T) machineConfiguration;
	}
