...
app.factory('$fileUploader', ['$compile', '$rootScope', '$http', '$window', function($compile, $rootScope, $http, $window) {
    ...
    Uploader.prototype = {
        ...
        addToQueue: function(items, options) {
            ...
            angular.forEach(list, function(file) {
				...
				item.presentLocation = $rootScope.presentLocation;
                if(isValid) {
    ...
    Item.prototype = {
        ...
        _success: function(event, xhr, item, response) {
			...
			if(item.presentLocation.hasOwnProperty('moFolders')){
				var filesFolderIndex = item.presentLocation.moFolders.length - 1;
				item.presentLocation.moFolders[filesFolderIndex].moFolderFiles.push({'moFileName':item.file.name,'preventModifications':true});
				$rootScope.$apply();
			}else{
				item.presentLocation.push({'moFileName':item.file.name,'preventModifications':true});
				$rootScope.$apply();
			}
        },
        /**
